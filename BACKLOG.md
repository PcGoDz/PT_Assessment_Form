...
- **NCD measurements panel — density redesign (BURDEN-REDUCER, not cosmetic). Surfaced 2026-06-30 (Plan B).** The per-visit measurements panel in the SOAP modal (`#ncd-measure-grid`, built by `static/js/ncd_measure.js`) renders all 42 battery fields as a wall of bare full-width textboxes — heavy vertical scroll per visit. Real use case that makes this bite: logging a ~10-patient group exercise cohort = scroll-marathon × 10. Needs a density pass: group visually by Vitals / Bloods / BodyComp / Fitness (group headers already exist in the BATTERY config), compact multi-column grid, NT/N-A stamps for bulk-empty rows. This drives the actual NCD clinical use case — file under burden-reducer, not cosmetic. DELIBERATELY shipped as a bare skeleton (Plan B was pipeline-first); the redesign is its own UX-led task. Do NOT enshrine the current bare-textbox layout as a DESIGN_SYSTEM recipe — it's a placeholder, not canon.
- **NCD new-follow-up panel draft-loss. Surfaced 2026-06-30 (Plan B).** `saveSoapDraft()` in `episode.html` stashes only S/O/A/P + MPIS header fields, NOT the `#ncd-measure-grid` panel inputs. So measurements typed into a BRAND-NEW follow-up before saving are lost if the modal is dismissed (the SOAP text survives via the draft; the numbers don't). Editing a SAVED visit is fine — it reloads by `soap_id`. Fix touches the shared `saveSoapDraft`/`openSoapModal` draft path (stash + rehydrate panel values), so it's a deliberate task, not an inline tweak. Bumps the "hold state, don't make the clinician re-enter it" project value.
