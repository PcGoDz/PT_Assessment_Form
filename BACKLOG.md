...
- **pdf_vestibular.py dead `BLACK`/`LGREY` imports + unused `baseline_label` parameter on `_battery_block()` (2026-07-19, cosmetic).** Both imported/passed but never read. Baked into the vetted plan's own code block, not implementer sloppiness. Clean up in a future pass, non-blocking.
