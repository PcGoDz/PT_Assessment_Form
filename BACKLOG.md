...
- **Dev-only "Fill dummy data" button — build before LYMPHOEDEMA/PAEDIATRIC/GENERAL.** One-click populates all form fields via the form's `populate()` for smoke-testing without manual data entry. MUST be dev-gated (flag-conditional; never ships in the clinical .exe). Build once, reuse across the 3 unbuilt forms. Supersedes/expands the `seed_db.py` item above — in-form button is the smoother path for the browser smoke loop. VESTIBULAR deliberately skipped (built manually, ship sailed).
...
- **VESTIBULAR battery chip overflow on window resize (deferred — future UI/UX polish pass).** Chip rows don't reflow cleanly when the browser window is resized narrow — chips overflow their container. Surfaced this session. Fix alongside KIV-toggle affordance and Head-Thrusts layout in the UI/UX pass.
- **pdf_vestibular.py AROM/PROM + Somatosensory + Coordination render as bordered grid tables; the borang shows dotted-line inline fills (2026-07-19).** Same visual-fidelity gap as the app-wide 2-column layout item above — revisit together in the PDF polish pass, not a VESTIBULAR-specific fix.
