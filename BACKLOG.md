...
- **NCD Plan A SHIPPED 2026-06-27** (merged `74c0991`). `ready=True`. 11-section form (MSK-canonical), PDF with body-shape PNG WYSIWYG embed, MPIS SOAPIER builder, clinical templates. **Plan B next**: per-visit `ncd_measurements` v3 table + trend page + additive SOAP-modal injection. Plan at `docs/superpowers/plans/2026-06-25-ncd-form-plan-B.md`. Battery keys FROZEN (see `form_ncd.js collect()` comment block — Plan B imports verbatim). Session_no→soap_id-FK alignment fix baked into Plan B Task 1–2.
...
- **DESIGN_SYSTEM gap — "textbox galore" / +Note collapsible pattern (surfaced 2026-06-27).** Forms with many always-visible free-text textareas feel heavy (flagged on NCD, BURN, HAND — 3 consumers = pattern gap, not per-form). The `+Note` collapsible pattern already exists form-locally in FACIAL and SCI. Proposed fix: (1) add a DESIGN_SYSTEM.md rule preferring `+Note` for OPTIONAL free-text, (2) promote the `+Note` markup to a documented component recipe, (3) apply across NCD/BURN/HAND. Own spec→plan→build cycle. **Promotion candidate** (3 confirmed consumers). Do NOT build speculatively — write the recipe first, then sweep.
