...
### High Priority
- [ ] NEURO form (high clinical volume, POMR exists)
- [ ] Geriatric duplicate RN/IC fields cleanup (low effort, good hygiene)
- [ ] Full end-to-end test of exe build (all 5 forms)
- [ ] Push to GitHub

### Medium Priority
- [ ] HAND form (simpler scope, good warmup before NEURO)
- [ ] Validation layer — hard stop on required fields (GPT senior dev is right on this one)
- [ ] Draft vs Final state for assessment records

### Lower Priority
- [ ] Versioning UI (audit_log data exists, no UI yet)
- [ ] Remaining 10 forms: HAND, BURN, NEURO, SCI, VESTIBULAR,
      FACIAL, PAEDIATRIC, LYMPHOEDEMA, NCD, GENERAL
- [ ] POMR-aligned MPIS output for assessment forms (currently uses assessment format,
      should match dept Word POMR template — Tarikh, Masa, Nombor Giliran at top)
