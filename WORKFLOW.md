...
- **When a worktree branch is behind main, `git diff --stat main` shows housekeeping noise — diff against the merge-base instead.** If main advances (e.g. a housekeeping commit) after a worktree branches off, `git diff --stat main` includes those unrelated files and can false-trigger a "shared-code leaked in" STOP. The true build footprint is `git diff --stat $(git merge-base <branch> main) <branch>`. Caught 2026-06-23 FACIAL +Note audit: stat-vs-main showed 4 files (.gitignore + FACIAL_UIX_SPEC.md noise from housekeeping `dc2f348`), stat-vs-merge-base showed the real 2 (facial.html + form_facial.js).
...
- **New schema migrations must use `PRAGMA user_version` gates, not blind ALTER-and-swallow.** Pattern: read version, gate each batch with `if version < N`, keep inner `try/except sqlite3.OperationalError` for mid-air DBs, stamp to target version last. See `database.py` lines 79–108 (v2). Trust version numbers cleanly from v2 onward.
- **`.git/config` corruption from worktree churn → `fatal: bad config line N`.** Two forms seen 2026-06-12: (a) trailing whitespace-only line, (b) null-byte (`\x00`) padding after a rewrite. Commits safe in `.git/objects`; config is only settings. Fix Windows-side: rewrite whole file fresh via `open('.git/config','wb').write(content.encode('utf-8'))`, NOT append/trim (avoids torn tails). Verify: `python -c "print(b'\x00' in open('.git/config','rb').read())"` → `False`. Cowork sandbox config reads can lag behind CC's Windows fix — tiebreak via CC `git status`.
- **When CSS spacing/sizing "looks off but the number's right," you're tuning against an overridden rule — read the COMPUTED style, not the source file.** A `.card-body` margin tweak was tuned three times against `.card-body { padding:18px }` (style.css:453) when the form runs inside `<main class="m3-main">`, so the more-specific `.m3-main .card-body { padding: var(--sp-5) var(--sp-6) }` (style.css:1809, = 20px) actually applied. The cascade picks the most-specific matching selector; if two rules target the same class, grep is not enough — open the element's Computed tab / box-model in DevTools and read the REAL applied value before doing margin/padding math. Cost: 4 cosmetic commits for one button (2026-06-11). For symmetric vertical gaps: top gap = card-body top padding + element top-margin; bottom gap = element bottom-margin (if the next sibling has no top-margin). Negative top-margin is the legitimate lever to claw back shared padding you must not edit.
- **Reusing `.irr-chip` for a non-irritability single-select requires form-local `.sel-<Value>` CSS — `style.css` does not define it.** `style.css` only defines `.sel-High/.sel-Medium/.sel-Low`. Borrowing `.irr-chip` for a new control (e.g., Affected Side R/L in FACIAL) correctly applies `.sel-R`/`.sel-L` via JS — the classes appear in the DOM — but no CSS rule matches, so the selected state silently never paints. No console error; DevTools shows the class present with no matching rule. Fix: add `.irr-chip.sel-R, .irr-chip.sel-L { background: var(--accent-light); border-color: var(--accent); color: var(--accent); font-weight: 500; }` (or equivalent) in the form's own `<style>` block inside `{% block content %}`. Do NOT add to `style.css` (axiom-protected shared file). Pattern: for every `.irr-chip` variant in a new form, grep `style.css` for each `.sel-<Value>` the JS applies — if absent, the rule must live form-locally. Caught FACIAL smoke-test 2026-06-17, fixed at commit `412ae7d`.
- **"Double marker + grey bleed" is TWO bugs that co-occur, not one.** When a body-chart click produces duplicate markers AND swapping forms shows greyed foreign markers, do not treat them as a single fault. Two independent root causes: (a) DOUBLE-ENTRY = `BodyChart.init()` bound twice (form template calls it in DOMContentLoaded AND main.js auto-fires it on `#svg-ant` detection) → one click fires `placeMarker` twice. Fix: remove the redundant template-side `init()`; rely on main.js auto-init (mirror ms.html, which has NO template init call). (b) GREY BLEED = `get_episode_record` cross-loads another form's record (Session M, fixed via form-type guard in `initFormContext`; residual limitation tracked in BACKLOG `get_episode_record form-aware`). First seen together on BURN, which made them read as one bug — the shared word "marker" is the trap. Diagnose which half: duplicate-on-single-click-same-session = (a); foreign markers on form-swap = (b). NCD hit (a) alone 2026-06-29 (redundant init at ncd.html DOMContentLoaded, fixed 1f27298).
- **A form-type guard on a shared-modal injection must cover EVERY entry path the modal opens through — new-item AND edit-existing — not just the one the happy-path test hits.** The shared SOAP modal (`episode.html`) opens two ways: `openSoapModal()` with no arg (new note) and with a `soap` arg (edit saved note). NCD Plan B's measurements panel guarded `maybeShow` by `form_type==='NCD'` but left the `loadForSoap` prefill fetch behind only `if (soap && soap.id)` — so editing an EXISTING *non-NCD* note fired a stray `GET /ncd-measurements`. Harmless (empty result, panel stays hidden) but it violates "a non-NCD form makes ZERO measurement calls." The new-note path passed the hands-on RED-LINE gate because no `soap.id` meant the fetch never fired; the edit path was the unguarded hole. Caught by re-reading the literal criterion ("zero stray calls") against own work, fixed `49293fe` (gate the fetch behind `isNcd` too). RULE: when guarding an additive injection into a shared modal by form type, enumerate every code path that opens the modal and guard each one — a happy-path test that only exercises the "new" path will green-light a leak on the "edit" path.
